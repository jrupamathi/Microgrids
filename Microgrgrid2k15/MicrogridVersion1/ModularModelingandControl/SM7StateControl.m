classdef SM7StateControl < Module
    methods
        function this = SM7StateControl(IndexName,RefFrameAngle,RefFrameSpeed)
            ParameterNames={'Lad','Laf','Laq','Ldf','LSd','LSq','LRD','LF','LRQ','RS','Rkd','RF','H','B','Rkq'};
            ...
            this.SetPoints =  sym(strcat(SetPointNames,IndexName,'_ref')); assume(this.SetPoints,'real');
            this.SetPoints = [this.SetPoints,sym(strcat(ControllableInputNames,IndexName,'_ref'))];assume(this.SetPoints,'real');
            this.PortStateTypes = {'Current','Current'};
            [this.StateSpace,this.ControlInputEquations] = SM7StateControl.SM7StateControlDynamics(this);
        end
    end
    methods(Static)
        function [StateSpace ControlInputEquations] = SM7StateControlDynamics(this)
            ...
            rq = this.Parameters(15);
            
            Tm = this.ControllableInputs(1);
            vf = this.ControllableInputs(2);
            
            K1 = this.ControllerGains(1);
            K2 = this.ControllerGains(2);
            K3 = this.ControllerGains(3);
            
            delta_ref = this.SetPoints(1);
            omega_ref = this.SetPoints(2);
            iF_ref = this.SetPoints(3);
            
            Tmeq = this.SetPoints(end-1);
            vfeq = this.SetPoints(end);
            ...
            domegadt=-(Id*vds - Tm +F*(omega-omega0) + Iq*vqs)/(2*H);

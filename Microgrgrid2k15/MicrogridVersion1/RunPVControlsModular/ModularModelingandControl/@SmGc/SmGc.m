%synchronous machine with governor control but no exciter control

classdef SmGc < Module
    methods
        function this = SmGc(IndexName,RefFrameAngle,RefFrameSpeed)
            ParameterNames={'LR','LS','LSS','M','RR','RS','J','B','vR','Tu','Tg'};
            ControllerGainNames={'Kt','r','Kp','Ki'};
            StateVariableNames = {'iSd','iSq','iR','omega','theta','tauL','a','omegaInt'};
            ...
            this.SetPoints =  sym(sym(strcat({'omega'},IndexName,'_ref')),'real');
            this.StateSpace = SmGc.SmGcDynamics(this.Parameters,this.StateVariables,this.ControllerGains,this.SetPoints,...
                                                                             this.PortInputs,this.RefFrameAngle,this.RefFrameSpeed);
        end
    end
    methods(Static)
        function StateSpace = SmGcDynamics(Parameters,StateVariables,ControllerGains,SetPoints,PortInputs,phi,dphidt)
            %Inputs____________________________________________
            %StateVariables: iSd,iSq,iR,omega,theta,tauL,a,omegaInt
            %PortInputs: vSd,vSq
            %Parameters: LR,LS,LSS,M,RR,RS,J,B,vR,Tu,Tg
            %ControllerGains: Kt,r,Kp,Ki
            %Set Points: omega_ref
            ...
            %StateSpace: [diSddt ; diSqdt ; diRdt ; domegadt ; dthetadt ; dtauLdt ; dadt ; domegaIntdt]

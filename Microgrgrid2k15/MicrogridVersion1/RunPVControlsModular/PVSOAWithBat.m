...
tic
[t,x]=ode45(@MilosSmTlWorking,[0,0.5],x0);
time  = toc
...
iq = iPVq_PV21;
            ...
            ud = ((-RPV*id_star ))/Vs +vd + dphidt*iq;
            uq = ((-RPV*iq_star ))/Vs +vq + -dphidt*id;
id = iPVd_PV21; iq = iPVq_PV21;
            %PV Dynamics
            diPVd_PV21dt = 377*(dphidt*iq - (RPV*id - vd + (Vs*ud))/LPV);
            diPVq_PV21dt = 377*(-dphidt*id - (RPV*iq -vq + (Vs*uq))/LPV);
            dVsdt = -(id*ud + iq*uq)/CPV + iDC/CPV - Vs/(100*RPV);

            Pb = 0.4; Qb = 0.3;
            vBd = vTLRd_TL_1_21; vBq = vTLRq_TL_1_21;
iBd_star = (Pb*vBd + Qb*vBq)/(vBd^2 + vBq^2);
iBq_star = (Pb*vBq - Qb*vBd)/(vBd^2 + vBq^2);

            %Battery
 uBd = ((-RPV*iBd_star ))/vBDC +vBd + dphidt*iBq;
            uBq = ((-RPV*iBq_star ))/vBDC +vBq + -dphidt*iBd;

            V0 = 1; ESR = RPV;
            iBDC = -(vBDC-V0)/ESR;
           diBddt = 377*(dphidt*iBd - (RPV*iBd - vBd + (vBDC*uBd))/LPV);
            diBqdt = 377*(-dphidt*id - (RPV*iq -vBq + (vBDC*uBq))/LPV);
            dvBDCdt = -(iBd*ud + iBq*uq)/CPV + iBDC/CPV - vBDC/(100*ESR);
